...
public class AlphaXSpread : Control, IDisposable
{
    ...
    public SpreadHitTestResult HitTest(Point point)
    {
        if (SheetViews.ActiveSheetView != null)
        {
            ...
            return point.X >= rowHeaderWidth && point.Y >= 0 && point.Y < columnHeaderHeight && point.X < SheetViewPane.RenderSize.Width
                ? SheetViewPane.ColumnHeadersRegion.HitTest(SheetViewPane.TranslatePoint(point, SheetViewPane.ColumnHeadersRegion))
                : point.X < rowHeaderWidth && point.Y < columnHeaderHeight
                ? SheetViewPane.TopLeftRegion.HitTest(SheetViewPane.TranslatePoint(point, SheetViewPane.TopLeftRegion))
                : null;
        }
    ...
    internal void RaiseCellsSelectionChanged(CellsSelectionEventArgs args) => CellsSelectionChanged?.Invoke(this, args);

    internal void RaiseCalculationError(CalcErrorEventArgs args) => CalculationError?.Invoke(this, args);
    #endregion
