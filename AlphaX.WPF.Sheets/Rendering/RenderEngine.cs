...
internal class RenderEngine : IRenderEngine, IDisposable
{
    ...
    public void EnsureNewCacheDrawing(Renderer renderer, int row, int col) => _cache.RemoveFromCache(renderer, row, col);
    #endregion

    public void DrawGridLines(int topRow, int leftCol, int bottomRow, int rightCol) => GridLinesRenderer.Render(topRow, leftCol, bottomRow, rightCol);

    public void DrawCellRange(int topRow, int leftColumn, int bottomRow, int rightColumn) => CellsRenderer.Render(topRow, leftColumn, bottomRow, rightColumn);
