...
internal class ViewPort : IViewPort
{
    ...
    public Rect GetColumnRect(int column) => new(_columns.GetLocation(column), _actualBounds.Top,
            _columns.GetColumnWidth(column), _actualBounds.Height);

    public Rect GetRowRect(int row) => new(_actualBounds.Left, _rows.GetLocation(row),
            _actualBounds.Width, _rows.GetRowHeight(row));

    public Rect GetRangeRect(CellRange range) => GetRangeRect(range.TopRow, range.LeftColumn, range.BottomRow, range.RightColumn);
    ...
    public Rect GetViewRangeRect() => GetRangeRect(ViewRange);
    ...
    /// <returns></returns>
    private bool IsTopRow(int row, double rowLocation) => _sheetView.ScrollPosition.Y >= rowLocation &&
    ...
    /// <returns></returns>
    private bool IsLeftColumn(int column, double colLocation) => _sheetView.ScrollPosition.X >= colLocation &&
            _sheetView.ScrollPosition.X < colLocation + _columns.GetColumnWidth(column);

    private bool GetIsEmpty() => _sheetView.WorkSheet.RowCount == 0 || _sheetView.WorkSheet.ColumnCount == 0;

    public override string ToString() => $"TopRow:{ViewRange.TopRow}, BottomRow:{ViewRange.BottomRow}, LeftColumn:{ViewRange.LeftColumn}, RightColumn:{ViewRange.RightColumn}";
}
